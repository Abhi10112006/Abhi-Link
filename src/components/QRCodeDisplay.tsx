...
export const QRCodeDisplay: React.FC<QRCodeDisplayProps> = ({
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText(buildPaymentRequestUrl().url);
      setLinkCopied(true);
          ...
          <div className="flex flex-col gap-3 w-full">
            ...
            <div className="flex gap-2 w-full">
              <motion.button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-[#f5f5f0] text-[#2d2d2b] border-2 border-[#d9d3ce] px-4 py-3 rounded-xl font-bold text-sm uppercase tracking-wide shadow-sm hover:bg-[#e6e1dc] transition-all overflow-hidden relative"
